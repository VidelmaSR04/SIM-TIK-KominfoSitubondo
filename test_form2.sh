...
# Save the submit response for debugging
echo "$SUBMIT_RESPONSE" > submit_response_debug.html

# Check if the submit response is a redirect (by looking for Location header in the response headers)
# We didn't capture headers, so we'll check the response body for a redirect to dashboard
# or we can check the HTTP status code by using -i to include headers
# Let's do a separate request to get the redirect URL
# But for now, let's check the response body for a sign of success or error
if echo "$SUBMIT_RESPONSE" | grep -q "Laravel"; then
  echo "Submit response contains Laravel debug page - likely an error"
  # Save the submit response for debugging
  echo "$SUBMIT_RESPONSE" > submit_error_debug.html
else
  echo "Submit response does not contain Laravel debug page"
fi

# Follow the redirect to dashboarduser
# We'll get the redirect URL from the Location header by using -i to include headers
