    ...
    <nav aria-label="Breadcrumb" class="flex text-sm text-secondary mb-4 font-body-md">
        <ol class="inline-flex items-center space-x-1 md:space-x-3">
            ...
            <li><span class="material-symbols-outlined text-sm mx-1">chevron_right</span><span
                    class="text-on-surface">Edit Pengguna</span></li>
        </ol>
    </nav>

    <h1 class="font-headline-lg text-headline-lg text-on-surface mb-6">Edit Pengguna</h1>
                ...
                <div class="grid grid-cols-1 gap-6">
                    <div>
                        ...
                        <input class="standard-input @error('name') border-red-500 @enderror" id="name" name="name"
                            placeholder="Masukkan nama lengkap" type="text" value="{{ old('name', $user->name) }}">
                        @error('name')
                    ...
                    <div>
                        ...
                        <input class="standard-input @error('email') border-red-500 @enderror" id="email" name="email"
                            placeholder="nama@situbondokab.go.id" type="email" value="{{ old('email', $user->email) }}">
                        @error('email')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input class="standard-input @error('password') border-red-500 @enderror" id="password"
                            name="password" placeholder="Kosongkan jika tidak ingin mengubah password" type="password">
                        <p class="helper-text">Biarkan kosong jika tidak ingin mengganti password.</p>
                        @error('password')
                            <p class="form-error">{{ $message }}</p>
                        @enderror
                    </div>
