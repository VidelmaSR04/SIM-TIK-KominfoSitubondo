...
<div class="flex items-center justify-between mb-6">
    <div>
        <h1 class="text-2xl font-bold text-on-surface">Manajemen cPanel</h1>
        <p class="text-secondary text-sm mt-1">Daftar pengelolaan cPanel dan domain.</p>
    </div>
...
<div class="bg-white rounded-xl border border-outline-variant shadow-sm overflow-hidden">
    ...
    <div class="table-container overflow-x-auto">
        <table class="w-full text-left border-collapse min-w-[900px]">
            ...
            <tbody class="text-sm text-on-surface divide-y divide-outline-variant/60">
                @php
                $cpanels = [
                    ['id' => 1, 'nama' => 'Kecamatan Arjasa', 'domain' => 'arjasa.situbondokab.go.id', 'local' => '192.168.99.72', 'public' => '103.165.156.249', 'status' => 'Active'],
                    ['id' => 2, 'nama' => 'Kecamatan Asembagus', 'domain' => 'asembagus.situbondokab.go.id', 'local' => '192.168.99.72', 'public' => '103.165.156.249', 'status' => 'Active'],
                    ['id' => 3, 'nama' => 'Bakesbangpol', 'domain' => 'bakesbangpol.situbondokab.go.id', 'local' => '192.168.99.72', 'public' => '103.76.175.182', 'status' => 'Active'],
                    ['id' => 4, 'nama' => 'Kecamatan Banyuglugur', 'domain' => 'banyuglugur.situbondokab.go.id', 'local' => '192.168.99.72', 'public' => '103.165.156.249', 'status' => 'Active'],
                    ['id' => 5, 'nama' => 'Kecamatan Banyuputih', 'domain' => 'banyuputih.situbondokab.go.id', 'local' => '192.168.99.72', 'public' => '103.165.156.249', 'status' => 'Active'],
                ];
                @endphp
                @foreach ($cpanels as $c)
                <tr class="hover:bg-gray-50 transition-colors">
                    <td class="p-4 text-gray-500">{{ $c['id'] }}</td>
                    <td class="p-4 font-medium text-on-surface">{{ $c['nama'] }}</td>
                    <td class="p-4 text-gray-500">{{ $c['domain'] }}</td>
                    <td class="p-4 font-mono text-xs text-gray-500">{{ $c['local'] }}</td>
                    <td class="p-4 font-mono text-xs text-gray-500">{{ $c['public'] }}</td>
                    <td class="p-4">
                        <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-700">
                            <span class="w-1.5 h-1.5 rounded-full bg-green-500"></span> {{ $c['status'] }}
                        </span>
                    </td>
                    <td class="p-4 text-center">
                        <div class="flex items-center justify-center gap-2 text-gray-500">
                            <button class="hover:text-teal-600 transition-colors"><span class="material-symbols-outlined text-[19px]">search</span></button>
                            <button class="hover:text-teal-600 transition-colors"><span class="material-symbols-outlined text-[19px]">edit</span></button>
                        </div>
                    </td>
                </tr>
                @endforeach
    ...
    <div class="p-4 border-t border-outline-variant flex flex-col sm:flex-row items-center justify-between gap-3 text-sm">
        <span class="text-gray-500">Menampilkan 1 hingga 5 dari 166 entri</span>
        <div class="flex gap-1">
            ...
            <button class="px-3 py-1.5 border border-outline-variant rounded-lg text-gray-500 hover:bg-gray-50 transition-colors">17</button>
