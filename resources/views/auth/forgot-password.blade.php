...
<head>
    ...
    <title>Lupa Password - SIM TIK</title>
    ...
    <div class="glass-card rounded-xl p-8 w-full relative overflow-hidden">
        ...
        <div class="flex flex-col items-center mb-8">
            <div class="w-20 h-20 mb-4 bg-surface-container rounded-full flex items-center justify-center p-2 shadow-sm border border-outline-variant">
                <span class="material-symbols-outlined text-primary" style="font-size: 40px;">lock_reset</span>
            </div>
            <h1 class="font-headline-lg text-headline-lg text-center text-on-surface mb-2">Reset Password</h1>
            <p class="font-body-md text-body-md text-on-surface-variant text-center">
                Masukkan email yang terdaftar dan password baru kamu. Password akan langsung diganti tanpa perlu verifikasi email.
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('password.email') }}" class="space-y-6">
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    ...
                    <input class="w-full pl-10 pr-4 py-2 bg-surface-container-lowest border border-outline-variant rounded-lg font-body-md text-body-md text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-shadow @error('email') border-red-500 @enderror"
                           ...
                           required autofocus autocomplete="username" placeholder="Masukkan email terdaftar"/>
                </div>
                @error('email')
            ...
            {{-- Password Baru --}}
            <div class="space-y-2">
                <label class="block font-label-md text-label-md text-on-surface" for="password">Password Baru</label>
                <div class="relative">
                    ...
                    <input class="w-full pl-10 pr-10 py-2 bg-surface-container-lowest border border-outline-variant rounded-lg font-body-md text-body-md text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-shadow @error('password') border-red-500 @enderror"
                           id="password" type="password" name="password" required autocomplete="new-password" placeholder="Masukkan password baru"/>
                    <button aria-label="Toggle password visibility" class="absolute right-3 top-1/2 -translate-y-1/2 text-outline hover:text-on-surface focus:outline-none" onclick="togglePassword('password', 'togglePasswordIcon')" type="button">
            ...
            <div class="space-y-2">
                <label class="block font-label-md text-label-md text-on-surface" for="password_confirmation">Konfirmasi Password Baru</label>
                <div class="relative">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-outline">lock</span>
                    <input class="w-full pl-10 pr-10 py-2 bg-surface-container-lowest border border-outline-variant rounded-lg font-body-md text-body-md text-on-surface focus:outline-none focus:ring-2 focus:ring-primary focus:border-primary transition-shadow"
                           id="password_confirmation" type="password" name="password_confirmation" required autocomplete="new-password" placeholder="Ulangi password baru"/>
                    <button aria-label="Toggle password visibility" class="absolute right-3 top-1/2 -translate-y-1/2 text-outline hover:text-on-surface focus:outline-none" onclick="togglePassword('password_confirmation', 'toggleConfirmIcon')" type="button">
                        <span class="material-symbols-outlined" id="toggleConfirmIcon">visibility</span>
                    </button>
                </div>
            </div>
